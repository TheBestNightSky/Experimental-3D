...
LRESULT Window::MsgHandler(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) noexcept {
	...
	case WM_KILLFOCUS:
		...
		break;
	case WM_ACTIVATE:
		OutputDebugStringA("activeate\n");
		// confine/free cursor on window to foreground/background if cursor disabled
		if (!cursorEnabled)
		{
			if (wParam & WA_ACTIVE)
			{
				OutputDebugStringA("activeate => confine\n");
				ConfineCursor();
				HideCursor();
			}
			else
			{
				OutputDebugStringA("activeate => free\n");
				FreeCursor();
				ShowCursor();
			}
		}
	...
	case WM_LBUTTONDOWN: {
		...
		if (!cursorEnabled)
		{
			OutputDebugStringA("lclick => recapture\n");
			ConfineCursor();
			HideCursor();
		}
		if (imio.WantCaptureMouse)
	...
	case WM_INPUT:
	{
		...
		// END OF RAW MOUSE EVENT MESSAGES //
	}
	}
